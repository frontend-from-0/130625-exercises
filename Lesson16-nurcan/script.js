...
let examWords = ["Kedi", "Maymun", "At", "Uğurböceği"];
filterLongWords(examWords, 7);
...
countOccurrences(["elma", "muz", "apple", "elma"]);
/*
...
console.log(averageArray([15, 22, 38, 40, 50]));
...
function flattenArray(twoDArray){
  ...
  for (let i = 0; i < twoDArray.length; i++) {
    for (let j = 0; j < twoDArray[i].length; j++) {
      NewTwoArray.push(twoDArray[i][j]);
    }
}
...
let words = ["istanbul", "ankara", "izmir", "adana"];
...
function updateTodoList(todoList, startIndex, deleteCount, ...newTasks) {
  console.log( "current taks:", todoList);
