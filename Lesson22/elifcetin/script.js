...
function validateName(name, firstNameError) {
  if (!onlyLetters.test(name)) {
    //if input DOESNT contain letters,
    firstNameError.innerText = "only letters are allowed";
    firstNameError.classList.remove("hidden");
    formValid = false;
  } else if (!(name.trim().length > 0)) {
    // if this is not true, then error message is shown
    firstNameError.classList.remove("hidden");
...
function validateCard(number) {
  if (!cardPattern.test(number)) {
    cardNumberError.innerText =
      "Please enter valid card number 0000 0000 0000 0000";
...
function validateAll() {
  ...
  if (fieldsFilled && !hasErrors) {
    // if all fields are filled, AND errors dont contain class hidden
...
cardNumberInput.addEventListener("input", () =>
  validateCard(cardNumberInput.value)
);
cardExpirationInput.addEventListener("input", () =>
  validateExpressionDate(cardExpirationInput.value)
);
