...
containsWord('<word> found in sentence.', 'found');
...
function compareNumbers(a, b) {
  if (typeof a !== 'number' || typeof b !== 'number') {
    console.log('Please provide numeric values');
    return;
  }
  if (a > b) {
...
censorWord('This is a bad word. Very bad.', 'bad');
...
function gradeChecker(score) {
  // Using true allows us to evaluate expressions inside cases
  switch (true) {
...
replaceCharacter('hello world', 'l', 'z');
...
titleCase('this is a title case sentence');
...
isSpam('Buy our SPAM free product!');
isSpam('Hello friend');
...
function containsNumber(str) {
  ...
  for (let i = 0; i < str.length; i++) {
    // If parsed integer is a number and not a space
...
padString('Hello', 10);
padString('Longer string than length', 10);
...
reverseWords('Hello world'); // Expected output: "olleH dlrow"
...
findWordPosition('Look for the word here', 'word');
findWordPosition('Look for the word here', 'missing');
